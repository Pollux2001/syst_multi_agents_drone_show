...
# Ground robots are not used in this air-writing case study.
nbTb3B = 0
Tb3B_pose = []
...
# Use 3 RMTT + 3 CF2 = 6 aerial luminous points.
# This respects the comments in tp_algos.py:
# RMTT max = 4, CF2 max = 3.
nbRMTT = 3
RMTT_pose = [
    [-0.90, -2.40, 0.0],
    [ 0.00, -2.40, 0.0],
...
# No obstacle for the air-writing pattern.
nbObstacle = 0
obstacle_size = []
obstacle_pose = []
...
# Set to 0.0 so that the air-writing pattern is clean and repeatable.
DRONE_POS_NOISE_STD = 0.0
...
ax.set_title("Six-Drone Air-Writing Simulation (Consensus + Formation)")
ax.set_xlabel('X (m)')
ax.set_ylabel('Y (m)')
ax.set_zlabel('Z (m)')
...
tb3B_plots = [
    ax.plot([], [], [], marker='s', color='blue', linestyle='',
            label='TB3 Burger' if _ == 0 else '')[0]
    for _ in range(nbTb3B)
]

tb3W_plots = [
    ax.plot([], [], [], marker='D', color='cyan', linestyle='',
            label='TB3 Waffle' if _ == 0 else '')[0]
    for _ in range(nbTb3W)
]

rmtt_plots = [
    ax.plot([], [], [], marker='^', color='orange', linestyle='',
            label='RMTT Drone' if _ == 0 else '')[0]
    for _ in range(nbRMTT)
]

cf2_plots = [
    ax.plot([], [], [], marker='*', color='green', linestyle='',
            label='CF2 Drone' if _ == 0 else '')[0]
    for _ in range(nbCF2)
]

rmep_plots = [
    ax.plot([], [], [], marker='h', color='magenta', linestyle='',
            label='RMEP Robot' if _ == 0 else '')[0]
    for _ in range(nbRMEP)
]
...
# Trajectory trails for air-writing visualization.
rmtt_trails = [[[], [], []] for _ in range(nbRMTT)]
cf2_trails  = [[[], [], []] for _ in range(nbCF2)]

rmtt_trail_lines = [
    ax.plot([], [], [], linestyle='-', linewidth=1.5, alpha=0.75, color='orange')[0]
    for _ in range(nbRMTT)
]

cf2_trail_lines = [
    ax.plot([], [], [], linestyle='-', linewidth=1.5, alpha=0.75, color='green')[0]
    for _ in range(nbCF2)
]
...
def get_async_cmd(robot_type, idx, default_cmd, func, *args):
    """Runs a robot's controller in the background. If it blocks, returns the last known command."""
...
def update(frame):
    ...
    for i in range(nbTb3B):
        ...
        vx, vy = get_async_cmd(
            'tb3B', i, default, tp_algos.tb3B_controller,
            i+1, pose.copy(), tb3B_snap, tb3W_snap, rmtt_snap, cf2_snap,
            rmep_snap, obs_poses, obs_sizes, [], clock_time
        )
    ...
    for i in range(nbTb3W):
        ...
        vx, vy = get_async_cmd(
            'tb3W', i, default, tp_algos.tb3W_controller,
            i+1, pose.copy(), tb3B_snap, tb3W_snap, rmtt_snap, cf2_snap,
            rmep_snap, obs_poses, obs_sizes, [], clock_time
        )
    ...
    for i in range(nbRMEP):
        ...
        vx, vy, wz = get_async_cmd(
            'rmep', i, default, tp_algos.rmep_controller,
            i+1, pose.copy(), tb3B_snap, tb3W_snap, rmtt_snap, cf2_snap,
            rmep_snap, obs_poses, obs_sizes, clock_time
        )
    ...
    for rob in all_robots:
        ...
        # Store and draw trails only for flying drones.
        if name.startswith("RMTT"):
            rmtt_trails[idx][0].append(cx)
            rmtt_trails[idx][1].append(cy)
            rmtt_trails[idx][2].append(cz)

            rmtt_trail_lines[idx].set_data(rmtt_trails[idx][0], rmtt_trails[idx][1])
            rmtt_trail_lines[idx].set_3d_properties(rmtt_trails[idx][2])

        elif name.startswith("CF2"):
            cf2_trails[idx][0].append(cx)
            cf2_trails[idx][1].append(cy)
            cf2_trails[idx][2].append(cz)

            cf2_trail_lines[idx].set_data(cf2_trails[idx][0], cf2_trails[idx][1])
            cf2_trail_lines[idx].set_3d_properties(cf2_trails[idx][2])

    clock_time += dt

    return (
        tb3B_plots + tb3W_plots + rmtt_plots + cf2_plots + rmep_plots
        + rmtt_trail_lines + cf2_trail_lines
    )
