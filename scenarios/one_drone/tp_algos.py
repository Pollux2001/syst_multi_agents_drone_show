...
def cf2_control_fn(robotNo, robotPose, tb3B_poses, tb3W_poses, rmtt_poses, cf2_poses, rmep_poses, obstacle_pose, obstacle_size, clock):
    ...
    if not TAKEOFF_DONE and robotPose[2] < 0.05: # if the drone is on the ground and takeoff is not done
        ...
        z_dist = 1.4  # target altitude (m) — CF2 uses altitude command, not vz
        t = max(0.0, clock - T_INIT - 3.0)  # 3s hover after takeoff before circling
        theta = 0.4 * t

        goal_x = 1.0 * math.cos(theta)
        goal_y = 1.0 * math.sin(theta)

        ex = goal_x - robotPose[0]
        ey = goal_y - robotPose[1]

        p, i, d = 0.5, 0.005, 0.05
        vx = velocity(ex, p, i, d, clock, 5)
        vy = velocity(ey, p, i, d, clock, 5)
