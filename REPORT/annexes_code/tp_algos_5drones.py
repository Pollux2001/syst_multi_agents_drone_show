...
def cf2_control_fn(robotNo, robotPose, tb3B_poses, tb3W_poses, rmtt_poses, cf2_poses, rmep_poses, obstacle_pose, obstacle_size, clock):
    ...
    ROTATION_DURATION = 12.0   # seconds for one full 360deg rotation
    ...
    # u_i = K_FORM * ?_{j?i} [(p_j-p_i) - (t_j-t_i)]  ? shape term
    ...
    # Force proportional to (1/dist ? 1/SAFE_DIST) -> grows fast as dist -> 0
    # ============================================================
    def repulsion():
        vx_r = vy_r = vz_r = 0.0
        min_d = float('inf')
    ...
    # x = VERT_R*cos(?_i)*cos(?),  y = VERT_R*cos(?_i)*sin(?),  z = z_hover + VERT_Z_OFFSET + VERT_R*sin(?_i)
    # ============================================================
    elif phase == 8:
