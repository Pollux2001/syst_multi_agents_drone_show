...
\begin{itemize}
	...
	\item Il est conseillé de numéroter les équations, les tableaux, les figures etc. et de les citer dans le texte. Par exemple, la variable $\textbf{x}$ est introduite dans (\ref{eq:x}) de la manière suivante : 
	\begin{equation}
	\label{eq:x}
...
\begin{itemize}
	\item Les scalaires sont en italique et en minuscule ;
	\item Les vecteurs sont en gras et en minuscule ;
	\item Les matrices sont en gras et en majuscule (par exemple).
...
\begin{itemize}
	...
	\item Pour ne pas couper les paragraphes avec des figures, faire référence au N\textsuperscript{\underline{\scriptsize o}} figure dans le texte (ex. ”\textit{voir} Figure \ref{fig:smile}” plutôt que ”\textit{on voit sur la figure ci-dessous} :”).
...
\begin{itemize}
	...
	\item Toutes les références bibliographiques sont numérotées et doivent être citées dans le document, par exemple \cite{robo}.
\end{itemize}
